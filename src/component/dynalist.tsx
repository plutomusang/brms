...
const Dynalist: React.FC<IDynalist>=(props)=>{
  ...
  const spSetUsers=async(id:number, name: string)=> {
    ...
    const itemClick =(id: number, data: string)=> {
      chkValueSet(!chkValue);
      inputValueSet(data);
      setUserID(id);
    ...
    return (
      <React.Fragment >
        ...
        <div className="dynalink">
            ...
            <ul className='list'>
              { cacheData.Set1.map( (values, index) => {
                  return (
              
                    <li className={'options ' + values.active} key={index}>
                        <div className="item"   onClick={()=>itemClick(values.id, values.name)}>{values.name }</div>
